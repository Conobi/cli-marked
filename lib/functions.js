...
function numberedLine(indent, line, number) {
  return isPointedLine(line, indent) ? {
    num: number + 1,
    line: line.replace(BULLET_POINT, numberedPoint(number + 1)),
  } : {
    num: number,
    line: toSpaces(numberedPoint(number)) + line,
  };
}
...
function numberedLines(lines, indent) {
  ...
  return lines.split('\n')
    .filter(identity)
    .map((line) => {
      const numbered = transform(line, number);
      number = numbered.num;
...
function list(body, ordered, indent) {
  let bodyClean = body.trim();
  bodyClean = ordered
    ? numberedLines(bodyClean, indent)
    : bulletPointLines(bodyClean, indent);
  return bodyClean;
}
